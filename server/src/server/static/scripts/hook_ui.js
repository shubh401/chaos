(() => {
	...
	function textSelector(elementId) {
		try {
			...
			range.__selectNodeContents(document.__getElementById(elementId));
			selection.__removeAllRanges();
			selection.__addRange(range);
		} catch (e) {
	...
	function simulateScrollEvent(whereTo = "bottom") {
		try {
			if (whereTo === "top")
				__scroll({ top: 0, left: 0, behavior: "smooth" });
	...
	function singleClick(elementIds, navigationElements) {
		try {
			document.body.__focus();
			document.body.__click();

			for (let elementId of elementIds) {
				try {
					document.__getElementById(elementId)?.__focus();
					document.__getElementById(elementId)?.__click();
				} catch (e) {
	...
	function elementTextSelector(elementId) {
		...
		try {
			if (inputElements.__has(elementId)) {
				document.__getElementById(elementId)?.__select();
			} else {
	...
	window.onload = function () {
		...
		console.__log(
			...
			performance.__now() - start,
			"ms"
		);

		start = performance.__now();
		simulateInputEvent();
